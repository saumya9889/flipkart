...
const MenuCard = ({
  ...
  return (
    <div
      className={`card ${isDropdownOpen ? "open" : ""}`}
      style={{ border: "none" }}
    >
      ...
      >
        <ul className="py-2 px-0 text-sm text-gray-700 dark:text-gray-200">
          {menuData?.map((item, index) => (
            <li
              key={index}
              className="w-full hover:bg-[#9ca2ad63] transition-all hover:transition-all flex items-center flex-1"
            >
              <a
                href="#"
      ...
      <div
        className="card-content border-none"
        onClick={(e) => toggleDropdown(title)}
      >
